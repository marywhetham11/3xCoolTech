...
"""
This file defines all requirement tests for R6.4.
...
class FrontEndBuyR6(BaseCase):
    ...
    def test_nameDoesNotExist(self, *_):
        """
        This function tests that if the user enters a ticket name that does not
        exist in the database into the buy form, then an error message should be 
        ...
        self.assert_text("Ticket name is not in database", "#message")
    ...
    def test_quantityTooHigh(self, *_):
        """
        This function tests that if the user enters a ticket quantity that is higher
        that the number of tickets available into the buy form, then an error message 
        should be displayed on the homepage
        """
        # logout to invalidate any logged-in sessions that may exist
        self.open(base_url +'/logout')
        # open login page
        ...
        # test if the error message is displayed
        self.assert_element("#message")
        self.assert_text("Quantity format is incorrect: There are only " + str(test_tickets[0].quantity) + " tickets avalible. Please enter a lower quantity", "#message")
