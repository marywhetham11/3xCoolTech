...
"""
This file defines all requirement tests for R5.5.
...
class FrontEndUpdateR5(BaseCase):

    @patch('qa327.backend.login_user', return_value=test_user)
    @patch('qa327.backend.get_account_balance', return_value=test_account_balance)
    @patch('qa327.backend.get_all_tickets', return_value=test_tickets)
    @patch('qa327.backend.get_user', return_value=test_user)
    def test_dateNonNumeric(self, *_):
        """
        This function tests that if the user enters a date into the update
        form that is not numeric, then an error message is displayed in the 
    ...
    @patch('qa327.backend.get_all_tickets', return_value=test_tickets)
    @patch('qa327.backend.get_user', return_value=test_user)
    def test_dateNotLength8(self, *_):
        """
        This function tests that if the user enters a date into the update
        form that does not have a length of 8, then an error message is 
        displayed in the message element on the homepage
        """
        # open logout page
        self.open(base_url + '/logout')
        # open login page
